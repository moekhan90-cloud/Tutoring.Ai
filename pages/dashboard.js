...
export async function getServerSideProps(ctx) {
  ...
  // topic stats last 50 attempts
  const recent = await prisma.attempt.findMany({
    where: { user: { email: session.user.email } },
    orderBy: { createdAt: 'desc' },
    take: 50,
    select: { id:true, questionResults: { select: { topic:true, correct:true } } }
  });

  const topicMap = new Map();
  recent.forEach(a => {
    a.questionResults.forEach(r => {
      const key = r.topic || '—';
...
export default function Dashboard({ user, topicStats }) {
  const acc = user.attempts.length
    ...
    <main className="container" style={{ paddingTop: 24 }}>
      ...
      <div className="card" style={{ marginTop: 12 }}>
        <div style={{ display:'grid', gap:12, gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))' }}>
          ...
          <Stat label="Avg accuracy" value={`${acc}%`} />
          <Stat label="Share link" value={<a href={`/share/${user.publicKey}`}>View public dashboard</a>} />
        </div>
      </div>
...
// expose options for getServerSession
export const authOptions = {
  ...require('./api/auth/[...nextauth]').default.authOptions
};
